...
### 可用模型（仅文本/通用模型）
- `qwen-team/deepseek-v3.2` — TokenPlan DeepSeek（默认，推荐）
- `qwen-team/qwen3.6-plus` — TokenPlan Qwen 3.6（可看图）
- `qwen-team/glm-5` — TokenPlan GLM-5
- `qwen-team/MiniMax-M2.5` — TokenPlan MiniMax M2.5
- `xdclaw-pool/kimi-k2.5` — Kimi（保底）
- `xdclaw-pool/mimo-v2-pro` — MiMo（保留）
- `xdclaw-pool/deepseek-reasoner` — DeepSeek 推理模型

### ⚠️ 严禁切到图片生成模型
以下模型**不能**作为主会话聊天模型切换：
- `qwen-team/qwen-image-2.0`
- `qwen-team/qwen-image-2.0-pro`
- `qwen-team/wan2.7-image`
- `qwen-team/wan2.7-image-pro`

原因：这些是图片生成模型，不适合普通文本会话；一旦切过去，会把当前 session 污染，后续文本消息容易 400。

### 示例
- 用户说"换个模型" → `session_status(model: "qwen-team/qwen3.6-plus")`
- 用户说"切回默认" → `session_status(model: "default")`
- 用户说"用推理模型" → `session_status(model: "xdclaw-pool/deepseek-reasoner")`
- 用户说"换成稳定点的" → `session_status(model: "qwen-team/glm-5")`
...
**主会话职责**：理解用户、拆解任务、派活、接收结果、最终回复用户。

**执行任务必须隔离**：搜索、批量处理、写文件、长链路调试等重活不要塞进主会话，必须放进独立 task/sub-agent session；主会话只保留任务摘要和最终结论，避免 session swamp。

### 什么时候 spawn sub agent

| 场景 | 做法 |
|------|------|
| 单一简单问答 | 主会话直接回答 |
| 多个独立任务 | 最多同时 spawn 2 个；其余排队 |
| 需要 10+ 次工具调用 | spawn sub agent 单独跑 |
| 长时间搜索/研究/写报告 | spawn sub agent，结果写文件 |
| 用户连续纠错/返工 | 主会话先复述目标，再派一个新 task 处理 |

### 怎么用

```python
sessions_spawn(
    task="把任务背景、限制、输出路径写清楚；结果保存到 tmp/xxx.md，不要直接回复用户",
    mode="run",
    label="清晰任务名"
)

sessions_yield("等待任务完成后，由主会话汇总并回复用户")
```

### 关键规则

1. **每个任务一个独立 session**
   - task 里必须写清上下文、目标、限制、输出路径
   - 不依赖主会话历史；不要让 sub agent 猜

2. **sub agent 只产出结果，不直接对用户说话**
   - 结果写 `tmp/...` 或返回给主会话
   - 最终回复必须由主会话统一发出，避免多 agent 回复混乱

3. **并发默认最多 2 个，活跃/易卡容器最多 1 个**
   - 不要一次 spawn 3+ 个任务
   - 新任务来了，如果已有任务在跑，先告诉用户“我先处理前一个，后一个排队”

4. **失败最多两轮，必须停下来汇报**
   - 同一任务同一方案失败 2 次：停止，不再换关键词/换模型硬重试
   - 遇到 429、timeout、模型不可用：最多 fallback 1 次；仍失败就回主会话说明原因
   - 如果判断是当前模型不适合，要明确告诉用户："当前模型卡住/限流/不适合这个任务，建议切到 XXX 模型继续。"
   - 用户同意后再执行模型切换；可以用 `session_status(model: "模型名")` 切换，或告诉用户发送 `/model 模型名`
   - 切模型后从主会话重新派一个新 task，明确不要沿用上一轮错误状态
   - 禁止 silent retry、禁止无限 fallback、禁止让用户以为还在正常处理

5. **进度必须回主会话**
   - 派活时：告诉用户“正在处理/已拆成几个任务”
   - 失败时：告诉用户卡在哪、已尝试什么、下一步需要用户还是人工协助
   - 完成时：主会话读取结果并统一回复

6. **状态写文件，不堆在 session 里**
   - 关键结果写入 `tmp/`、`memory/` 或项目文件
   - 不要让一个聊天 session 背大量历史和 checkpoint

### 错误示例（绝对禁止）

❌ 主会话连续执行多个长任务，导致上下文膨胀
❌ 一次 spawn 3 个以上 sub agent
❌ sub agent 直接给用户发多条结论，主会话不汇总
❌ 失败后不断换模型/换关键词/换工具继续试
❌ 用户已纠错时，仍沿用旧 session 的错误状态继续做

### 正确示例

✅ 用户："查 A、B 两家公司"
→ spawn 2 个 sub agent，每个查一家
→ 结果写文件
→ 主会话汇总后回复

✅ 用户："这个结果不对，重新按 skill 来"
→ 主会话先复述新要求
→ spawn 一个新 task，明确“不要沿用上一轮错误结论”
→ 完成后主会话回复
...
<!-- xdclaw-image-rules-baseline:start -->

## 🎨 生图 / 图生图规则（Xdclaw 公共基线）

用户要求生成图片、画图、海报、头像、写真、婚纱照、棚拍、换背景、按这张图生成、图生图、改图、重绘、换风格时，必须使用 `skills/multimodal-image`，不要只给提示词，不要只文字描述，不要说不能生成。

生图/图生图可能需要 2-3 分钟。QQBot 网关层会优先把明确的生图/图生图请求改为异步任务并先回复任务ID；如果请求已经进入 agent 并需要手动调用 generate/edit，调用后要等待工具完成，不要因为 60 秒未返回就结束或重复调用。若最终失败，按失败防循环规则最多重试一次。

返回图片时，优先输出 QQBot 可识别的媒体标签，例如：

```text
<qqmedia>/tmp/xdclaw-images/xxx.png</qqmedia>
```

同一个请求最多尝试 2 次：第一次失败可修正明显参数问题后重试一次；第二次仍失败必须停止并说明具体错误，不要第三次调用，不要伪造图片路径。

`gpt-image-2` 的 `/images/edits` 端点已用 QQ 实际 JPG 文件复测成功。遇到一次 502/上游错误时，不要声称不支持 edits 或“上游没配置”；按失败防循环规则最多重试一次，仍失败才返回具体错误。

<!-- xdclaw-image-rules-baseline:end -->
